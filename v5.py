...
def main():
    ...
    output_json_path = 'extracted_statement_data_v5.json'
    ...
    try:
        ...
        # Extract tables using camelot - try different approaches
        print("Extracting tables...")
        tables = []
        
        # Try different camelot extraction methods
        try:
            # Method 1: Stream with basic settings
            tables_stream = camelot.read_pdf(pdf_path, pages='all', flavor='stream', password=password)
        ...
        try:
            # Method 2: Lattice method (for tables with borders)
            tables_lattice = camelot.read_pdf(pdf_path, pages='all', flavor='lattice', password=password)
        ...
        try:
            ...
            tables_edge = camelot.read_pdf(pdf_path, pages='all', flavor='stream', 
                                         edge_tol=500, password=password)
            tables.extend(tables_edge)
            print(f"Edge detection method found {len(tables_edge)} tables")
        except Exception as e:
